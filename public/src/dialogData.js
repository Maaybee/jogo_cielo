...
export function validateDialogStructure(scenario, scenarioName) {
  ...
  scenarioKeys.forEach((key) => {
    ...
    if (step.playerOptions) {
      step.playerOptions.forEach((option, index) => {
        // 1. Checa se faltou alguma propriedade obrigatória
        if (
          !option.id ||
          !option.text ||
          typeof option.isCorrect !== "boolean" ||
          typeof option.satisfaction !== "number" ||
          !option.nextDialog
        ) {
          console.error(
            `[Erro na Matriz] Cenário ${scenarioName}, step '${key}', Opção ${index}: Faltam propriedades ou os tipos estão errados.`,
          );
        }

        // 2. Checa se o próximo diálogo existe (evita tela travada/morta)
        const destinationExists =
          scenarioKeys.includes(option.nextDialog) ||
          allowedGlobalKeys.includes(option.nextDialog);
        if (!destinationExists) {
          console.warn(
            `[Caminho Quebrado] Cenário ${scenarioName}, step '${key}': O nextDialog aponta para '${option.nextDialog}', mas esse passo não existe!`,
          );
        }
      });
    }
...
validateDialogStructure(OSWALDO_DIALOG, "Oswaldo");
validateDialogStructure(JULIE_DIALOG, "Julie");
validateDialogStructure(JOSE_DIALOG, "José");
validateDialogStructure(TXORI_DIALOG, "Txori");
